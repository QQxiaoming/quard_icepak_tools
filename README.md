...
- 支持按块名规则定义多条“块组细化规则”，对不同块组施加不同的局部网格参数。
- 等待网格生成完成后，统计 `Quality`、`Skewness`、`Face alignment`、`Cell volume` 四项指标。
- 在独立结果窗口中展示各指标的最小值和最大值。
- 结果报告会额外给出每条局部细化规则的命中 block 数量与覆盖项摘要。

块组细化规则说明：

- 每条规则支持 `精确匹配 / 通配匹配 / 正则匹配` 三种块名匹配方式。
- 优先级越大，规则越晚应用；后应用的规则只覆盖自己显式填写的局部参数。
- 当前首批支持的局部覆盖项包括：局部尺寸 `X/Y/Z`、局部分离间隙 `X/Y/Z`、棱柱层开关、平滑质量阈值、平滑迭代次数、特征细化开关、全部窄缝包含开关。
...
- 工具的 `key + version` 组合必须唯一；相同 `key` 的不同版本可以同时存在。
