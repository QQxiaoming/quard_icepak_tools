...
作者本人只是一个程序员，并不懂流体仿真，但是我老婆是个CFD工程师，因为他工作中经常需要重复性批量处理模型或仿真参数、枚举分析结果，手动修改非常麻烦且易错，所以我就开发了这款自动化工具管理器，将她常用的操作收集为一个个子工具，然后统一在一个界面里管理和运行，里面的任务控制流都是她多年从业的实际经验总结设计的，虽然目前工具数量还不多，但后续会持续增加。这个工具作为送给她的礼物。
